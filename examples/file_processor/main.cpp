...
int main(int argc, char** argv) {
    ...
    auto& count = app.option<int>("-c,--count", "Number of lines to process").default_value(10);
    auto& names = app.multi_option<std::string>("-n,--names", "Names (repeat -n)").required();

    auto& input  = app.positional<std::string>("input", "Input file");
    auto& output = app.positional<std::string>("output", "Output file").default_value("output.txt");
